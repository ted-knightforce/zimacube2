...
printf "    [1/4] Seq Write  : " && grep -A30 "arc-seqwrite: (groupid=0" $RESULTS | grep -m1 "WRITE: bw="
printf "    [2/4] Rand Write : " && grep -A30 "arc-randwrite: (groupid=0" $RESULTS | grep -m1 "WRITE: bw="
echo ""
echo -e "${YELLOW}  Reads:${NC}"
printf "    [Warmup]         : " && grep -A30 "arc-warmup: (groupid=0" $RESULTS | grep -m1 "READ: bw="
printf "    [3/4] Warm ARC   : " && grep -A30 "arc-randread-warm: (groupid=0" $RESULTS | grep -m1 "READ: bw="
printf "    [4/4] Cold       : " && grep -A30 "arc-randread-cold: (groupid=0" $RESULTS | grep -m1 "READ: bw="
