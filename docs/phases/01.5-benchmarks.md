...
|---|---|---|
| 16GB single-channel (original) | ~8–10GB | Hot Docker databases, small working sets stay in RAM |
| **32GB dual-channel (current ✅)** | ~20–24GB | Immich thumbnails + databases + Ollama model pages cached simultaneously — and **+51% measured warm ARC IOPS** from the dual-channel memory bandwidth (see [Phase 1.8](#phase-18-p510-onboard-migration-june-25-2026)) |
...
> - 🌨️ **Arctic-Storage** — Crucial P510 2TB PCIe 5.0 btrfs (onboard M.2 since Phase 1.8) — random IOPS, sub-ms latency
...
> - ⚙️ **Kingston 256GB** — OS drive (`nvme4n1` after the Phase 1.8 reshuffle) — ZimaOS system only

### Phase 1 — Foundation (current)

| Workload | Pool | Reason |
|---|---|---|
| ZimaOS system + boot | Kingston OS drive | OS isolation; survives pool wipes |
...
| NVIDIA drivers / CUDA libs (Phase 4b) | Kingston OS drive | System-level install; stays on OS volume |
...
| ⚙️ Kingston OS drive | OS only | ZimaOS boot, NVIDIA drivers (Phase 4b) |
...
### What Happened When I Moved the P510 to the Onboard Slot ✅

I expected the onboard M.2 slot to unlock native PCIe Gen5 speeds — sequential read jumping from 874 MB/s to ~9,000+ MB/s. That's not what happened, and the real answer is more interesting. Full results in [Phase 1.8 below](#phase-18-p510-onboard-migration-june-25-2026); the short version: I roughly doubled the bandwidth, but the slot turned out to be a single-lane-and-a-half compromise, not a Gen5 firehose.

---

## Phase 1.8 — P510 Onboard Migration (June 25, 2026)

I physically relocated the Crucial P510 from the 7th Bay (behind the ASMedia bridge) to the ZimaCube 2's onboard M.2 slot, then re-ran `benchmark-arctic.sh` to see what changed. The goal was to escape the 7th Bay's 800 MB/s cap and find out whether the onboard slot could feed the P510's PCIe 5.0 potential.

### Before vs After — Arctic-Storage (btrfs)

| Test | Before — 7th Bay | After — Onboard M.2 | Change |
|---|---|---|---|
| Sequential write | 788 MB/s | **1,231 MB/s** | 🟢 +56% |
| Sequential read | 874 MB/s | **1,677 MB/s** | 🟢 +92% |
| Random 4K write | 87,099 IOPS | 59,682 IOPS | 🔴 −31%* |
| Random 4K read | 205,588 IOPS | **403,078 IOPS** | 🟢 +96% |
| Random 4K read latency | 0.6 ms | **0.32 ms** | 🟢 ~47% lower |

\* The random-write drop is **not** an apples-to-apples comparison. The original benchmark ran on a near-empty drive; the P510 now holds ~853 GB (43% full), which shrinks the dynamic SLC cache and raises write amplification. This run also followed a 72 GiB sequential write that may have left the SLC cache partially saturated. To isolate the slot's true effect on random write, the test should be re-run on a quiesced drive after 15+ minutes idle.

### The PCIe Link — Why It's Not Gen5

I expected native Gen5. The drive's link tells the real story:

```bash
# P510 is nvme5n1 after the move (nodes reshuffled — see note below)
cat /sys/class/nvme/nvme5/device/max_link_speed   # 32.0 GT/s PCIe  (drive's own Gen5 capability)
cat /sys/class/nvme/nvme5/device/max_link_width   # 4               (drive can do x4)
cat /sys/class/nvme/nvme5/device/current_link_speed  # 8.0 GT/s PCIe (idle, ASPM-downtrained)
cat /sys/class/nvme/nvme5/device/current_link_width  # 2             (only 2 lanes routed)
```

The key reading is **`current_link_width = 2`**. ASPM power-saving downtrains link *speed* at idle but never reduces *width* — so 2 lanes is what the slot physically provides. The `max` values (Gen5 x4) are just the P510 advertising its own capability; the slot doesn't deliver it.

| | Drive capability | Onboard slot delivers | 7th Bay delivered |
|---|---|---|---|
| Link | PCIe 5.0 x4 | **PCIe 3.0 x2** | ASMedia bridge, ~Gen3 x1 |
| Bandwidth ceiling | ~10,000 MB/s | **~1,970 MB/s** | ~800 MB/s |
| Measured seq read | — | 1,758 MB/s (89% of ceiling) | 874 MB/s |

The measured 1,758 MB/s sitting at 89% of the PCIe 3.0 x2 ceiling confirms the slot operates at Gen3 x2 — a healthy, saturated 2-lane link, not a broken one.

### Verdict

Moving to the onboard slot **roughly doubled the available bandwidth** (one lane → two) and delivered ~2× sequential, ~2× random read, and lower latency — a genuine, worthwhile upgrade for everything Arctic-Storage actually does (Docker container reads, database lookups, Immich thumbnail serving, model loading). But the honest headline stands: **no slot on the ZimaCube 2 Standard exposes more than ~2 GB/s to this drive.** The fast PCIe lanes go to OCuLink Slot 1 (Gen4 x4 — which is why glacier flies) and the boot M.2; the secondary onboard M.2 gets chipset-attached Gen3 x2. The P510's Gen5 ceiling stays locked in this chassis — which was always fine, since it was bought on price, not peak speed.

> 💡 **Device-node names are not stable.** Moving the P510 reshuffled every `nvmeXn1` name (the P510 went from `nvme0n1` to `nvme5n1`, the Kingston boot drive from `nvme5n1` to `nvme4n1`, and the glacier drives shifted down). The kernel assigns these at boot — they are **not** a reliable identifier. This is exactly why ZimaOS mounts btrfs by filesystem UUID and ZFS imports `glacier` by pool GUID: storage keeps working regardless of which node a drive lands on. Always identify a drive by **model + serial** (`nvme list`), never by node.

### Bonus finding — the 32GB dual-channel RAM upgrade also landed

Around the same time, the planned RAM upgrade went in (2× Corsair Vengeance 16GB = **32GB, dual-channel**, confirmed by ARC `c_max` rising to ~30 GiB). Re-running `benchmark-arc.sh` on glacier twice showed a clean, repeatable jump in warm ARC performance:

| Test | 16GB single-channel (7-run) | 32GB dual-channel (2-run) | Change |
|---|---|---|---|
| **Warm ARC random 4K read** | 83,416–84,113 IOPS @ 1.47–1.49 ms | **126,816–126,888 IOPS @ 1.01 ms** | 🟢 **+51% IOPS, −32% latency** |
| Warm-up sequential read | 3,329–4,459 MB/s | 4,182–4,711 MB/s | 🟢 Higher |
| Sequential write | 1,681–1,752 MB/s | 1,631–1,641 MB/s | ⚪ Flat |
| Random 4K write | 12,560–13,725 IOPS | 9,762–12,688 IOPS | ⚪ Flat (SLC variance across back-to-back runs) |
| Cold random 4K read | 16,062–18,315 IOPS @ 7–7.7 ms | ~15,600–15,900 IOPS @ ~8 ms | ⚪ Flat |
| Session ARC hit rate | 99.3–100% | 99.6% | ⚪ Same |

The isolation here is clean: **everything that touches the disk stayed flat** (writes hit the unchanged glacier pool; cold reads bypass ARC), and **only the RAM-served path jumped**. That pins the +51% on the memory subsystem — going single-channel → dual-channel roughly doubled memory bandwidth, and warm ARC random reads are memory-bandwidth-bound (`sys=95%` CPU, zero disk involvement). This is the direct, measured proof of the Phase 1 prediction that the 32GB upgrade would be a read-performance investment, not just a headroom one — and it adds a nuance: it's *dual-channel* RAM that feeds ARC, not just *more* RAM.

> One caveat from the second run: the cold-read headline showed 3,349 IOPS, but that figure was skewed by a single ~60-second I/O stall on the OCuLink pool mid-test. The per-sample average was ~15,600 IOPS, consistent with the established cold baseline — so the stall is a transient outlier, not a regression.
...
Phase 1.8 is done — and the answer was that the onboard slot is a Gen3 x2 link, not the Gen5 firehose I'd hoped for (full results above). The 32GB dual-channel RAM upgrade landed at the same time and delivered a measured +51% warm ARC read. Phase 2 (Jellyfin) is still on hold until the Seagate IronWolf drives finish arriving.

- **Phase 1.8** — Move P510 to onboard M.2 slot → re-benchmark ✅ complete — onboard slot is PCIe 3.0 x2 (~2× the 7th Bay), 32GB dual-channel RAM landed (+51% warm ARC)
...
- **Phase 4a** — Ollama CPU-only AI inference baseline on i3-1215U (32GB RAM now installed)
