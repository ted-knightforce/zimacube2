...
Measured using `benchmark-arc.sh` across seven runs. ZFS ARC does not cache write operations — warm ARC affects reads only. Write figures reflect steady-state `benchmark-glacier.sh` results with SLC cache intact.

| Test | Bandwidth | IOPS | Latency (avg) | Notes |
|---|---|---|---|---|
| Sequential write | 1,681–1,752 MiB/s | — | — | Cold pool throughput; ARC not involved |
| Random 4K write | — | 12,560–13,725 | — | Cold IOPS; ARC not involved |
| Sequential read (warm-up) | 3,329–4,459 MiB/s | — | — | ARC-buffered prefetch pass |
| **Random 4K read — Warm ARC** | **324–329 MiB/s** | **83,416–84,113** | **1.47–1.49 ms** | Served from RAM; 99.3–100% session hit rate |
| Random 4K read — Cold | 63–72 MiB/s | 16,062–18,315 | 7.05–7.72 ms | `--direct=1`; raw NVMe baseline |

> Warm ARC IOPS spans only **0.8% variance** across all seven runs regardless of starting ARC state or preceding write load. Average latency locked at 1.47–1.49 ms every run.
...
|---|---|---|---|---|---|---|---|
| 1 | 9.32 GiB | 3,329 MiB/s | 83,416 | 1.49 ms | 99.9% | 16,276 | 7.62 ms |
| 2 | 0.22 GiB | 4,298 MiB/s | 83,707 | 1.48 ms | 100.0% | 18,315 | — |
| 3 | 0.23 GiB | 4,459 MiB/s | 83,588 | 1.48 ms | 100.0% | 14,063 | — |
| 4 | 0.23 GiB | 4,328 MiB/s | 83,929 | 1.48 ms | 100.0% | 17,577 | 7.05 ms |
| 5 | 0.23 GiB | 3,832 MiB/s | 83,858 | 1.48 ms | 99.4% | 16,497 | 7.52 ms |
| 6 | 0.23 GiB | 4,063 MiB/s | 83,992 | 1.48 ms | 99.4% | 16,821 | 7.37 ms |
| 7 | 0.24 GiB | 4,098 MiB/s | 84,113 | 1.47 ms | 99.3% | 16,062 | 7.72 ms |
...
### Planned Phase 1.8 — Onboard M.2 Re-benchmark
